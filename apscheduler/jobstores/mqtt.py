...
class MqttMemoryJobStore(BaseJobStore):
    """
    ...
    def __init__(self, mqtt_client, jobs_topic):
        super(MqttMemoryJobStore, self).__init__()
        # list of (job, timestamp), sorted by next_run_time and job id (ascending)
        self.mqtt = mqtt_client
        self.jobs_topic = jobs_topic
        self.mqtt.subscribe(jobs_topic)
        self.mqtt.message_callback_add(
            jobs_topic,
            self.mqtt_job_callback)
        self._jobs = []
        self._jobs_index = {}  # id -> (job, timestamp) lookup table

    def jobs_topic_callback(self, mqtt, obj, msg):
        job_dict = json.loads(msg.payload)
        self._logger.info(job_dict)
        job_id = job_dict['id']
        if job_dict['trigger'] == 'date':
            trigger =
        misfire_grace_time = job_dict['misfire_grace_time']
        if trigger is "date":
            datetime = isodate.parse_datetime(job_dict['datetime'])
